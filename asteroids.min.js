...
this.navigation.innerHTML="(press esc to quit) <br />\n<input type=\"submit\" value=\"Reset\" onclick=\"window.ASTEROIDS.resurrect()\" />";this.gameContainer.appendChild(this.navigation);this.points=document.createElement('span');this.points.id='ASTEROIDS-POINTS';this.points.style.font="28pt Arial, sans-serif";this.points.style.fontWeight="bold";this.points.className="ASTEROIDSYEAH";this.navigation.appendChild(this.points);this.appstore=document.createElement('div');with(this.appstore.style){position='absolute';top='10px';right='10px';zIndex='9999999';}
...
window.ASTEROIDS.enemiesKilled++;if(!(this.dying[i].id)){this.dying[i].id="uid_"+uidNum;uidNum++;}
window.ASTEROIDS.deadStyles[this.dying[i].id]=this.dying[i].style.display;this.dying[i].style.display="none";}catch(e){}}
setScore();this.dying=[];}
...
else window.ASTEROIDSPLAYERS[window.ASTEROIDSPLAYERS.length]=new Asteroids();window.ASTEROIDS.resurrect=function(){var i;var wad=window.ASTEROIDS.deadStyles;for(i in wad){console.log([i,wad[i]]);document.getElementById(i).style.display=wad[i];}
